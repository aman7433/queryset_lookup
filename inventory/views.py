...
def home(request):
    ...
    # Product.objects.filter(added_on__date__range=(date(2024, 1, 1), date(2025, 6, 1)))
    # Product.objects.filter(added_on__date=date(2025, 6, 19))
    # Product.objects.filter(added_on__year=2025)
    # Product.objects.filter(added_on__year__gt=2024)
    # Product.objects.filter(added_on__month=6)
    # Product.objects.filter(added_on__week=25)
    # Product.objects.filter(added_on__week_day=4)  # Wednesday
    # Product.objects.filter(added_on__quarter=2)
    # Product.objects.filter(added_on__time__gt=time(12, 0))
    # Product.objects.filter(added_on__hour=10)
    # Product.objects.filter(added_on__minute__gt=30)
    # Product.objects.filter(added_on__second__lte=30)
    # Product.objects.filter(description__isnull=False)
    # Product.objects.filter(name__regex=r'^B.*')        # Starts with 'B'
    # Product.objects.filter(name__iregex=r'butter|milk')
